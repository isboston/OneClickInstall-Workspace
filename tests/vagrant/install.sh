...
function prepare_vm() {
  if [ -f /etc/lsb-release ]; then
    ...
    DISTRIB_CODENAME=$(grep "VERSION=" /etc/os-release | awk -F= '{ print $2 }' | sed 's/[0-9\"]//g' | sed 's/[() ]//g')
    DISTRIB_RELEASE=$(grep "VERSION_ID=" /etc/os-release | awk -F= '{ print $2 }' | sed 's/[0-9\"]//g' | sed 's/[() ]//g')
  fi
  ...
  # 🔶 RHEL 9 support (must come BEFORE Debian handling!)
...
EOF
  ...
  # 🔷 Debian/Ubuntu-specific logic
  if [ ! -f /etc/centos-release ] && grep -qi "debian\|ubuntu" /etc/os-release; then
    if [ "$DIST" = "debian" ]; then
  ...
  if [ -f /etc/centos-release ]; then
    ...
    if [[ "$CENTOS_REV" =~ ^9 ]]; then
      ...
      echo "${COLOR_GREEN}☑ PREPARE_VM: SHA1 GPG key policy enabled${COLOR_RESET}"
    else
...
END
  ...
  # 🧼 Clean home and prepare test env
  rm -rf /home/vagrant/*
  ...
  echo "${COLOR_GREEN}☑ PREPARE_VM: Hostname was set up${COLOR_RESET}"
}
